...
import pymechanize
assert pymechanize.__version__ >= (0, 0, 6, "a")

mech = pymechanize.Browser()
...
print("Found {0} {1}".format(len(urls), "tarballs to download"))
...
for url in urls:
    ...
    while 1:
        ...
        f.write(data)
    f.close()
    print("{0} {1}".format(os.stat(filename).st_size, "bytes"))
