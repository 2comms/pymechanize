...
request = pymechanize.Request(pymechanize.urljoin(uri, "mechanize/example.html"))
response = pymechanize.urlopen(request)
forms = pymechanize.ParseResponse(response, backwards_compat=False)
response.close()
# # f = open("example.html")
...
print("{0}{1}".format(control.name, control.value, control.type))
control.value = ["mascarpone", "curd"]

# ...and the Item instances from inside the Control
item = control.get("curd")

print("{0}{1}".format(item.name, item.selected, item.id, item.attrs))
item.selected = False
...
pymechanize.Item(cheeses, {"contents": "mascarpone",
                         "value": "mascarpone"})
...
try:
    response2 = pymechanize.urlopen(request2)
