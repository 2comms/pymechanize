...
import pymechanize
from pymechanize import CookieJar, HTTPCookieProcessor, \
...
from pymechanize.rfc3986 import urljoin
from pymechanize.util import hide_experimental_warnings, \
    reset_experimental_warnings, read_file, write_file
import pymechanize.opener
import pymechanize.rfc3986
import pymechanize.sockettimeout
...
# from pymechanize import CreateBSDDBCookieJar
...
class TestCase(pymechanize.testcase.TestCase):
    ...
    def make_browser(self):
        browser = pymechanize.Browser()
        self._configure_user_agent(browser)
        return browser

    def make_user_agent(self):
        ua = pymechanize.UserAgent()
        self._configure_user_agent(ua)
        return ua

    def build_opener(self, handlers=(), build_opener=None):
        handlers += (pymechanize.ProxyHandler(proxies={}),)
        if build_opener is None:
            build_opener = pymechanize.build_opener
        return build_opener(*handlers)

    def setUp(self):
        pymechanize.testcase.TestCase.setUp(self)
        self.test_uri = urljoin(self.uri, "test_fixtures")
        self.server = self.get_cached_fixture("server")
        if self.no_proxies:
            old_opener_m = pymechanize.opener.opener
            old_opener_u = urllib2._opener
            pymechanize.install_opener(pymechanize.build_opener(
                    pymechanize.ProxyHandler(proxies={})))
            urllib2.install_opener(urllib2.build_opener(
                    urllib2.ProxyHandler(proxies={})))
            def revert_install():
                pymechanize.install_opener(old_opener_m)
...
class SimpleTests(SocketTimeoutTest):
    ...
    def test_simple(self):
        ...
        # relative URL
        self.browser.open('/pymechanize/')
        self.assertEqual(self.browser.title(), 'pymechanize')

    def test_basic_auth(self):
        uri = urljoin(self.uri, "basic_auth")
        self.assertRaises(pymechanize.URLError, self.browser.open, uri)
    ...
    def test_urlopen_with_default_timeout(self):
        timeout_log = self._monkey_patch_socket()
        response = pymechanize.urlopen(self.test_uri)
    ...
    def test_urlopen_with_timeout(self):
        timeout_log = self._monkey_patch_socket()
        timeout = 10.
        response = pymechanize.urlopen(self.test_uri, timeout=timeout)
        self.assert_contains(response.read(), "Python bits")
        timeout_log.verify(timeout)
    ...
    def test_redirect_with_timeout(self):
        ...
        req = pymechanize.Request(urljoin(self.test_uri, "test_fixtures"),
                                timeout=timeout)
    ...
    def test_error_recovery(self):
        self.assertRaises(pymechanize.URLError, self.browser.open,
    ...
    def test_redirect(self):
        # 301 redirect due to missing final '/'
        codes = []
        class ObservingHandler(pymechanize.BaseHandler):
    ...
    def test_refresh(self):
        def refresh_request(seconds):
            ...
            val = urllib.quote('%d; url="%s"' % (seconds, self.uri))
    ...
    def test_open_novisit(self):
        def test_state(br):
            ...
            self.assertRaises(pymechanize.BrowserStateError, br.back)
        ...
        r = self.browser.open_novisit(pymechanize.Request(uri))
        test_state(self.browser)
        self.assert_("GeneralFAQ.html" in r.read(2048))
...
class ResponseTests(TestCase):
    ...
    def test_seekable_response_opener(self):
        build_opener = pymechanize.OpenerFactory(
            pymechanize.SeekableResponseOpener).build_opener
    ...
    def test_new_response(self):
        ...
        response = pymechanize.make_response(
            data,
...
#            HTTPResponseDebugProcessor(),
    ...
    def test_retrieve_to_named_file(self):
        url = urljoin(self.uri, "/pymechanize/")
    ...
    def test_urlretrieve(self):
        ...
        url = urljoin(self.uri, "/pymechanize/")
        verif = CallbackVerifier(self)
        filename, headers = pymechanize.urlretrieve(url,
    ...
    def test_reload_read_incomplete(self):
        ...
        r2 = browser.open(urljoin(self.uri, "pymechanize"))
...
# #         from pymechanize import CacheFTPHandler, build_opener
...
class FormsExamplesTests(pymechanize.testcase.GoldenTestCase):
...
class CookieJarTests(TestCase):
    ...
    def test_mozilla_cookiejar(self):
        ...
        def make_cookiejar():
            cj = pymechanize.MozillaCookieJar(filename=filename)
            try:
                cj.revert()
    ...
    def test_firefox3_cookiejar(self):
        ...
        def make_cookiejar():
            ...
            try:
                return pymechanize.Firefox3CookieJar(filename=filename)
            finally:
