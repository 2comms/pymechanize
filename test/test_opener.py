...
import pymechanize
import pymechanize.response as _response
...
class URLOpener(pymechanize.OpenerDirector):
...
class OpenerTests(unittest.TestCase):
    ...
    def test_retrieve_closes_on_failure(self):
        def fail_to_open():
            raise pymechanize.URLError("dummy reason")
    ...
    def test_retrieve(self):
        ...
        class Opener(pymechanize.OpenerDirector):
            def __init__(self, content_length=None):
                pymechanize.OpenerDirector.__init__(self)
                self.calls = []
                self.block_size = pymechanize.OpenerDirector.BLOCK_SIZE
                self.nr_blocks = 2.5
        ...
        try:
            ...
            except pymechanize.ContentTooShortError as exc:
