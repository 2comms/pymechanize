...
from pymechanize.urllib_fork import md5_digest

from test_tools import testprogram
...
class DigestAuthHandler:
    ...
    def handle_request(self, request_handler):
        ...
        if not request_handler.headers.has_key('Proxy-Authorization'):
            ...
            # MSIE uses short_path in its validation, but pymechanize uses the
...
class ProxyAuthTests(TestCase):
    ...
    def setUp(self):
        ...
        handler = pymechanize.ProxyHandler({"http" : proxy_url})
        self.proxy_digest_handler = pymechanize.ProxyDigestAuthHandler()
        self.opener = pymechanize.build_opener(handler, self.proxy_digest_handler)

    def test_proxy_with_bad_password_raises_httperror(self):
        self.proxy_digest_handler.add_password(self.REALM, self.URL,
                                               self.USER, self.PASSWD + "bad")
        self.assertRaises(pymechanize.HTTPError,
                          self.opener.open,
                          self.URL)
...
class TestUrlopen(TestCase):
    """Tests pymechanize.urlopen using the network.
    ...
    def test_redirection(self):
        ...
        f = pymechanize.urlopen('http://localhost:%s/' % handler.port)
        data = f.read()
        f.close()
    ...
    def test_404(self):
        ...
        try:
            pymechanize.urlopen('http://localhost:%s/weeble' % handler.port)
        except pymechanize.URLError as f:
            pass
        else:
    ...
    def test_200(self):
        ...
        f = pymechanize.urlopen('http://localhost:%s/bizarre' % handler.port)
        data = f.read()
        f.close()
    ...
    def test_sending_headers(self):
        ...
        req = pymechanize.Request("http://localhost:%s/" % handler.port,
                              headers={'Range': 'bytes=20-39'})
        pymechanize.urlopen(req)
        self.assertEqual(handler.received_headers['Range'], 'bytes=20-39')

    def test_basic(self):
        handler = self._make_request_handler([(200, [], "we don't care")])

        open_url = pymechanize.urlopen("http://localhost:%s" % handler.port)
    ...
    def test_bad_address(self):
        ...
        self.assertRaises(IOError,
                          ...
                          pymechanize.urlopen, "http://sadflkjsasf.i.nvali.d./")
