...
FACTORY_CLASSES = [pymechanize.DefaultFactory, pymechanize.RobustFactory]
...
def make_mock_handler(response_class=MockResponse):
    class MockHandler:
        ...
        def handle(self, fn_name, response, *args, **kwds):
            ...
            if response:
                if isinstance(response, pymechanize.HTTPError):
                    raise response
                r = response
                r.seek(0)
            else:
...
class TestBrowser(pymechanize.Browser):
    default_features = []
    default_others = []
    default_schemes = []

class TestBrowser2(pymechanize.Browser):
    # XXX better name!
...
""", {"content-type": "text/html"})
    ...
    def test_encoding(self):
        ...
        b = pymechanize.Browser()
        for s, ct in [("", pymechanize.html.DEFAULT_ENCODING),

                      ("Foo: Bar\r\n\r\n", pymechanize.html.DEFAULT_ENCODING),
    ...
    def test_history(self):
        ...
        class Handler(pymechanize.BaseHandler):
            def http_open(self, request):
                r = response.test_response(url=request.get_full_url())
        ...
        self.assertRaises(pymechanize.BrowserStateError, b.back)
        ...
        self.assertEquals(b.geturl(), "http://example.com/")
        self.assertRaises(pymechanize.BrowserStateError, b.back, 2)
        r8 = b.open("/spam")

        # even if we get an HTTPError, history, .response() and .request should
        # still get updated
        class Handler2(pymechanize.BaseHandler):
            def https_open(self, request):
                r = pymechanize.HTTPError(
                    "https://example.com/bad", 503, "Oops",
                    MockHeaders(), StringIO.StringIO())
                return r
        b.add_handler(Handler2())
        self.assertRaises(pymechanize.HTTPError, b.open,
    ...
    def test_viewing_html(self):
        ...
        for allow_xhtml in False, True:
            for ct, expect in [
                ...
                b = TestBrowser(pymechanize.DefaultFactory(
    ...
    def _test_empty(self, factory):
        ...
        self.assertRaises(pymechanize.BrowserStateError, b.open, "relative_ref")
        ...
        def test_state_error(method_names):
            for attr in method_names:
                ...
                # print attr
                self.assertRaises(pymechanize.BrowserStateError, method)
            self.assertRaises(pymechanize.BrowserStateError, b.select_form,
                              name="blah")
            self.assertRaises(pymechanize.BrowserStateError, b.find_link,
                              name="blah")
        # ...if not visiting a URL...
        test_state_error(("geturl reload back viewing_html encoding "
                          "click links forms title select_form".split()))
        self.assertRaises(pymechanize.BrowserStateError, b.set_cookie, "foo=bar")
        self.assertRaises(pymechanize.BrowserStateError, b.submit, nr=0)
        self.assertRaises(pymechanize.BrowserStateError, b.click_link, nr=0)
        self.assertRaises(pymechanize.BrowserStateError, b.follow_link, nr=0)
        self.assertRaises(pymechanize.BrowserStateError, b.find_link, nr=0)
...
""", {"content-type": "text/html"})
        ...
        self.assertRaises(pymechanize.FormNotFoundError, b.select_form,
                          name="blah")
        self.assertRaises(pymechanize.FormNotFoundError, b.select_form,
                          predicate=lambda form: form is not b.global_form())
        self.assertRaises(pymechanize.LinkNotFoundError, b.find_link,
                          name="blah")
        self.assertRaises(pymechanize.LinkNotFoundError, b.find_link,
...
"""
        ...
        self.assertRaises(pymechanize.FormNotFoundError, b.select_form, "foo")

        # no form is set yet
        self.assertRaises(AttributeError, getattr, b, "possible_items")
        b.select_form("form1")
        # now unknown methods are fed through to selected pymechanize.HTMLForm
        self.assertEqual(
    ...
    def _test_link_encoding(self, factory):
        import pymechanize
        from pymechanize.rfc3986 import clean_url
        url = "http://example.com/"
...
""", {"content-type": "text/html"})
        ...
        self.assertRaises(pymechanize.LinkNotFoundError,
...
""",
    ...
    def test_set_cookie(self):
        ...
        self.assertRaises(pymechanize.BrowserStateError,
...
class ResponseTests(TestCase):
    ...
    def test_set_response(self):
        import copy
        from pymechanize import response_seek_wrapper
    ...
    def test_str(self):
        ...
        response = response.response_seek_wrapper(
...
class HttplibTests(pymechanize.testcase.TestCase):
    ...
    def monkey_patch_httplib(self, putheader):
        ...
        def getresponse(self_):
            class Response(object):
                msg = get_headers(StringIO.StringIO(""))
                status = 200
                reason = "OK"
                def read(self):
                    return ""
    ...
    def test_add_host_header(self):
        ...
        request = pymechanize.Request("http://example.com/")
