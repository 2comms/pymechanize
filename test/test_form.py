...
import pymechanize
import pymechanize.form as form
from pymechanize import ControlNotFoundError, ItemNotFoundError, \
     ItemCountError, AmbiguityError
import pymechanize.testcase as _testcase
from pymechanize.util import get1
...
#     def raise_deprecations():
#         try:
#             registry = form.__warningregistry__
...
class DummyForm:
    ...
    def find_control(self, name, type):
        raise pymechanize.ControlNotFoundError


class UnescapeTests(unittest.TestCase):

    def test_unescape_charref(self):
        unescape_charref = form.unescape_charref
    ...
    def test_get_entitydefs(self):
        get_entitydefs = form.get_entitydefs
        ed = get_entitydefs()
        for name, char in [
            ("&amp;", u"&"),
            ("&lt;", u"<"),
...
""")  # "
        forms = pymechanize.ParseFile(file, "http://localhost/",
...
""")  # "
        forms = pymechanize.ParseFileEx(f, "http://localhost/", encoding="utf-8")
        form = forms[1]
        test_string = "&amp;" + (u"\u2014".encode('utf8') * 3)
...
class LWPFormTests(unittest.TestCase):
    ...
    def testEmptyParse(self):
        forms = pymechanize.ParseFile(StringIO(""), "http://localhost",
    ...
    def _forms(self):
        ...
        """)
        return pymechanize.ParseFile(file, "http://localhost/",
                                   backwards_compat=False)
...
#             self.assertRaises(
#                 pymechanize.ParseError,
#                 pymechanize.ParseFile, f, base_uri, backwards_compat=False,
#                 )
#             self.assert_(issubclass(pymechanize.ParseError,
...
""")
        base_uri = "http://localhost/"
        forms = pymechanize.ParseFile(f, base_uri, backwards_compat=False)
        form = forms[0]
        for ctl in form.controls:
            self.assert_(isinstance(ctl, form.TextControl))
...
""")
        base_uri = "http://localhost/"
        forms = pymechanize.ParseFileEx(f, base_uri)
        outer = forms[0]
...
"""),
        ...
        hide_deprecations()
        forms = pymechanize.ParseResponse(r)
        reset_deprecations()
...
"""),
        ...
        forms = pymechanize.ParseResponseEx(r)
        self.assertEqual(len(forms), 2)
        outer = forms[0]
        inner = forms[1]
    ...
    def test_ParseString(self):
        class DerivedRequest(pymechanize.Request):
            pass
        forms = pymechanize.ParseString('<input name="a" />',
                                  "http://example.com/",
...
""")
        ...
        try:
            pymechanize.ParseFile(f, base_uri, backwards_compat=False)
        except pymechanize.ParseError as e:
...
"""),
    ...
    def test_empty_document(self):
        forms = pymechanize.ParseFileEx(StringIO(""), "http://example.com/")
...
class ControlTests(unittest.TestCase):
    def testTextControl(self):
        attrs = {"type": "this is ignored",
                 ...
                 "maxlength": "20",
                 "id": "foo"}
        c = form.TextControl("texT", "ath_Uname", attrs)
        c.fixup()
        ...
        self.assertRaises(AttributeError, bad_assign)
        del attrs["disabled"]
        c = form.TextControl("hidden", "ath_Uname", attrs)
        self.assert_(c.readonly)
        self.assertRaises(AttributeError, bad_assign)

    def testFileControl(self):
        c = form.FileControl("file", "test_file", {})
        fp = StringIO()
        c.add_file(fp)
        fp2 = StringIO()
    ...
    def testIsindexControl(self):
        attrs = {"type": "this is ignored",
                 "prompt": ">>>"}
        c = form.IsindexControl("isIndex", None, attrs)
        c.fixup()
    ...
    def testIgnoreControl(self):
        attrs = {"type": "this is ignored"}
        c = form.IgnoreControl("reset", None, attrs)
    ...
    def testSubmitControl(self):
        attrs = {"type": "this is ignored",
                 ...
                 "value": "value_value",
                 "img": "foo.gif"}
        c = form.SubmitControl("submit", "name_value", attrs)
        ...
        # click on button
        form = form.HTMLForm("http://foo.bar.com/")
        c.add_to_form(form)
        self.assert_(c.pairs() == [])
    ...
    def testImageControl(self):
        attrs = {"type": "this is ignored",
                 "name": "name_value",
                 "img": "foo.gif"}
        c = form.ImageControl("image", "name_value", attrs, index=0)
    ...
    def testCheckboxControl(self):
        attrs = {"type": "this is ignored",
                 ...
                 "alt": "some string"}
        form = DummyForm()
        c = form.CheckboxControl("checkbox", "name_value", attrs)
        c.add_to_form(form)
        c.fixup()
        ...
        c2 = form.CheckboxControl("checkbox", "name_value", attrs2)
        c2.add_to_form(form)
        c.merge_control(c2)
        c.add_to_form(form)
        c.fixup()
    ...
    def testSelectControlMultiple(self):
        ...
        c = form.SelectControl("select", "select_name", attrs)
        c.add_to_form(form)
        c.fixup()
        ...
        c = form.SelectControl("select", "select_name", attrs, select_default=True)
        c.add_to_form(form)
        c.fixup()
        self.assert_(c.value == ["value_value"])
        ...
        attrs2 = attrs.copy()
        attrs2["value"] = "value_value2"
        c2 = form.SelectControl("select", "select_name", attrs2)
        c2.add_to_form(form)
        c.merge_control(c2)
        c.add_to_form(form)
        c.fixup()
...
# #         </SELECT>
        ...
        c = form.SelectControl("select", "select_name", attrs)
        c2 = form.SelectControl("select", "select_name", attrs2)
        c3 = form.SelectControl("select", "select_name", attrs3)
        form = DummyForm()
        c.merge_control(c2)
        c.merge_control(c3)
        c.add_to_form(form)
...
# #         </SELECT>
    ...
    def testRadioControl(self):
        attrs = {"type": "this is ignored",
                 ...
                 "id": "blah"}
        # Netscape and IE behaviour...
        c = form.RadioControl("radio", "name_value", attrs)
        form = DummyForm()
        c.add_to_form(form)
        c.fixup()
        ...
        attrs2 = attrs.copy()
        attrs2["value"] = "value_value2"
        c2 = form.RadioControl("radio", "name_value", attrs2,
                                select_default=True)
        c.merge_control(c2)
        c.add_to_form(form)
        c.fixup()
        ...
        attrs = {"type": "this is ignored",
                 ...
                 "value": "value_value",
                 "id": "name_value_1"}
        c1 = form.RadioControl("radio", "name_value", attrs)
        attrs = {"type": "this is ignored",
                 "name": "name_value",
                 "value": "value_value",
                 "id": "name_value_2",
                 "checked": "checked"}
        c2 = form.RadioControl("radio", "name_value", attrs)
        attrs = {"type": "this is ignored",
                 "name": "name_value",
                 "value": "another_value",
                 "id": "name_value_3",
                 "__label": {"__text": "Third Option"}}
        c3 = form.RadioControl("radio", "name_value", attrs)
        form = DummyForm()
        c1.merge_control(c2)
        c1.merge_control(c3)
        c1.add_to_form(form)
        ...
        form._id_to_labels['name_value_1'] = [
            form.Label({'for': 'name_value_1', '__text':'First Option'})]
        form._id_to_labels['name_value_2'] = [
            form.Label({'for': 'name_value_2',
                         '__text':'Second Option'})]
        form._id_to_labels['name_value_3'] = [
            form.Label({'for': 'name_value_3',
...
""")
        form = pymechanize.ParseFile(f, "http://example.com/",
...
"""
        ...
        self.assertRaises(
            AmbiguityError,
            form.find_control, type="checkbox", name=pymechanize.Missing)
        ctl = form.find_control(type="checkbox", name=pymechanize.Missing, nr=1)
...
""")
        form = pymechanize.ParseFile(file, "http://blah/",
                                   backwards_compat=False)[0]
...
""" % method)
    ...
    def testAuth(self):
        ...
        forms = pymechanize.ParseFile(fh, self.base_uri,
                                    backwards_compat=False)
        self.assert_(len(forms) == 1)
        form = forms[0]
...
""")
        for kwds, backwards_compat in [({}, True),
            ...
            hide_deprecations()
            form = pymechanize.ParseFile(f, "http://localhost/", **kwds)[0]
            reset_deprecations()
            f.seek(0)
...
""")
        form = pymechanize.ParseFile(f, "http://localhost/",
                                   backwards_compat=False)[0]
        ...
        self.assertRaises(pymechanize.AmbiguityError, c.get, label="Loaf")
        self.assertRaises(
            pymechanize.AmbiguityError, c.set_value_by_label, ["Loaf"])
        ...
        self.assertRaises(
            pymechanize.AmbiguityError, c.get, label="Loaf of Bread")
...
def make_form(html):
    global_form, form = pymechanize.ParseFileEx(StringIO(html),
                                              "http://example.com/")
...
def make_form_global(html):
    return get1(pymechanize.ParseFileEx(StringIO(html), "http://example.com/"))
...
""")
        form = pymechanize.ParseFile(f, "http://blah/",
                                   backwards_compat=False)[0]
        form["murphy"] = ["a", "b", "c"]
        form["woof"] = ["d"]
...
""")
        return pymechanize.ParseFile(f, "http://blah/",
                                   backwards_compat=False)[0]
...
""",
    ...
    def test_click_empty_form_by_label(self):
        # http://github.com/jjlee/pymechanize/issues#issue/16
        form = make_form_global("")
        assert len(form.controls) == 0
        self.assertRaises(pymechanize.ControlNotFoundError,
...
class FunctionTests(unittest.TestCase):
    ...
    def test_normalize_line_endings(self):
        def check(text, expected, self=self):
            got = form.normalize_line_endings(text)
...
"""
        ...
        return pymechanize.ParseFile(StringIO(html),
    ...
    def test_empty_upload(self):
        ...
        forms = pymechanize.ParseFile(StringIO("""<html>
...
<input type="submit" name="submit"></input>
    ...
    def test_no_files(self):
        # no files uploaded
        self.monkey_patch(form, "choose_boundary", lambda: "123")
        forms = pymechanize.ParseFileEx(StringIO("""<html>
