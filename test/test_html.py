...
class RegressionTests(TestCase):
    ...
    def test_bad_base_tag(self):
        ...
        for factory in [pymechanize.DefaultFactory(), pymechanize.RobustFactory()]:
            br = pymechanize.Browser(factory=factory)
    ...
    def test_robust_form_parser_uses_beautifulsoup(self):
        factory = pymechanize.RobustFormsFactory()
        self.assertIs(factory.form_parser_class,
                      pymechanize.form.RobustFormParser)

    def test_form_parser_does_not_use_beautifulsoup(self):
        factory = pymechanize.FormsFactory()
        self.assertIs(factory.form_parser_class, pymechanize.form.FormParser)
    ...
    def test_robust_form_parser_does_not_raise_on_bad_html(self):
        self._make_forms_from_bad_html(pymechanize.RobustFormsFactory())

    def test_form_parser_fails_on_bad_html(self):
        self.assertRaises(
            pymechanize.ParseError,
            self._make_forms_from_bad_html, pymechanize.FormsFactory())


class CachingGeneratorFunctionTests(TestCase):

    def _get_simple_cgenf(self, log):
        from pymechanize.html import CachingGeneratorFunction
        todo = []
        for ii in range(2):
            def work(ii=ii):
                log.append(ii)
...
class UnescapeTests(TestCase):
    ...
    def test_unescape_charref(self):
        from pymechanize.html import unescape_charref
    ...
    def test_unescape(self):
        ...
        from pymechanize.html import unescape
        data = "&amp; &lt; &mdash; &#8212; &#x2014;"
        mdash_utf8 = u"\u2014".encode("utf-8")
...
class EncodingFinderTests(TestCase):
    ...
    def make_response(self, encodings):
        return pymechanize.response.test_response(
            headers=[("Content-type", "text/html; charset=\"%s\"" % encoding)
                     for encoding in encodings])

    def test_known_encoding(self):
        encoding_finder = pymechanize.html.EncodingFinder("default")
