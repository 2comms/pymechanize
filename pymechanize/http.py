...
from . import sgmllib_copy as sgmllib
from .urllib_fork import HTTPError, BaseHandler

from .headersutil import is_html
from .html import unescape, unescape_charref
from .request import Request
from .response import response_seek_wrapper
from . import rfc3986, sockettimeout

debug = logging.getLogger("pymechanize").debug
debug_robots = logging.getLogger("pymechanize.robots").debug
...
class XHTMLCompatibleHeadParser(AbstractHeadParser, HTMLParser):
    def __init__(self):
...
class MechanizeRobotFileParser(robotparser.RobotFileParser):
    ...
    def __init__(self, url='', opener=None):
        ...
        self._opener = opener
        self._timeout = sockettimeout._GLOBAL_DEFAULT_TIMEOUT

    def set_opener(self, opener=None):
        from . import opener
        if opener is None:
            opener = opener.OpenerDirector()
        self._opener = opener
    ...
    def read(self):
        ...
        try:
            ...
            pass
        except (IOError, socket.error, OSError) as e:
...
class HTTPRefererProcessor(BaseHandler):
    ...
    There's a proper implementation of this in pymechanize.Browser.
...
def clean_refresh_url(url):
    ...
    if ((url.startswith('"') and url.endswith('"')) or
        ...
        url = url[1:-1]
    return rfc3986.clean_url(url, "latin-1")  # XXX encoding
