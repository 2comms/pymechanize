...
class SGMLParser(_markupbase.ParserBase):
    ...
    def finish_starttag(self, tag, attrs):
        try:
            method = getattr(self, "start_%s" % tag)
    ...
    def finish_endtag(self, tag):
        if not tag:
            ...
            if tag not in self.stack:
                try:
                    method = getattr(self, "end_%s" % tag)
                except AttributeError:
                    self.unknown_endtag(tag)
                else:
    ...
    def report_unbalanced(self, tag):
        if self.verbose:
            print ("*** Unbalanced </{0}{1}".format(tag, ">"))
            print ("*** Stack: {0}".format(self.stack))
...
class TestSGMLParser(SGMLParser):
    ...
    def handle_comment(self, data):
        ...
        r = repr(data)
        if len(r) > 68:
            r = "{0}{1}{2}".format(r[:32], '...', r[-32:])
        print("comment: %s" % r)

    def unknown_starttag(self, tag, attrs):
        self.flush()
        if not attrs:
            print("start tag: <{0}{1}".format(tag, ">"))
        else:
            print("start tag: <%s" % tag)
            for name, value in attrs:
                print("{0}{1}{2}{3}{4}".format(name, '=', '"', value, '"'))
            print('>')

    def unknown_endtag(self, tag):
        self.flush()
        print("end tag: </{0}{1}" .format(tag, ">"))

    def unknown_entityref(self, ref):
        self.flush()
        print("*** unknown entity ref: &{0}{1}".format(ref, ';'))

    def unknown_charref(self, ref):
        self.flush()
        print("*** unknown char ref: &#{0}{1}".format(ref, ';'))

    def unknown_decl(self, data):
        self.flush()
        print("*** unknown decl: [{0}{1}".format(data, ']'))
