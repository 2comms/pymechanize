...
class OpenerDirector(urllib_fork.OpenerDirector):
    def __init__(self):
        urllib_fork.OpenerDirector.__init__(self)
    ...
    def _request(self, url_or_req, data, visit,
        ...
        if isstringlike(url_or_req):
            ...
            # already a pymechanize.Request instance
            req = url_or_req
            if data is not None:
                req.add_data(data)
            # XXX yuck
    ...
    def open(self, fullurl, data=None,
        ...
        # call ._open() instead.
        urlopen = urllib_fork.OpenerDirector._open
    ...
    def error(self, proto, *args):
        if proto in ['http', 'https']:
            ...
            d = self.handle_error['http']  # https is not different than http
            proto = args[2]  # YUCK!
            ...
            http_err = 0
        args = (d, proto, meth_name) + args
        result = self._call_chain(*args)
        if result:
            return result

        if http_err:
            args = (d, 'default', 'http_error_default') + orig_args
    ...
    def retrieve(self, fullurl, filename=None, reporthook=None, data=None,
        ...
        try:
            ...
            if filename:
                tfp = openf(filename, 'wb')
            else:
                path = rfc3986.urlsplit(req.get_full_url())[2]
    ...
    def close(self):
        urllib_fork.OpenerDirector.close(self)
...
class SeekableResponseOpener(ResponseProcessingOpener):
    def process_response_object(self, response):
        return response.seek_wrapped_response(response)
...
class OpenerFactory:
    ...
    default_classes = [
        # handlers
        urllib_fork.ProxyHandler,
        urllib_fork.UnknownHandler,
        urllib_fork.HTTPHandler,
        urllib_fork.HTTPDefaultErrorHandler,
        urllib_fork.HTTPRedirectHandler,
        urllib_fork.FTPHandler,
        urllib_fork.FileHandler,
        # processors
        urllib_fork.HTTPCookieProcessor,
        urllib_fork.HTTPErrorProcessor,
        ]
    if hasattr(httplib, 'HTTPS'):
        default_classes.append(urllib_fork.HTTPSHandler)
    handlers = []
