...
the rest of pymechanize may be used instead.
...
import random
import re
import socket.SocketIO as socket
import sys
import time
...
# XXX: The urllib, urllib2, and urlparse
# XXX: modules have been combined in the
# XXX: urllib package in Python 3.
#
...
from urllib.request import url2pathname, getproxies

from urllib.error import HTTPError, URLError

from .clientcookie import CookieJar
from .response import closeable_response
from .sockettimeout import _GLOBAL_DEFAULT_TIMEOUT
from . import rfc3986, request
...
def sha1_digest(s):
    return hashlib.sha1(str.encode(s, 'utf-8')).hexdigest()

def md5_digest(s):
    return hashlib.md5(str.encode(s, 'utf-8')).hexdigest()

def redline_delegator(fh):
...
class Request:
    ...
    def get_selector(self):
        scheme, authority, path, query, fragment = rfc3986.urlsplit(
            self.__r_host)
        if path == "":
            path = "/"  # RFC 2616, section 3.2.2
        fragment = None  # RFC 3986, section 3.5
        return rfc3986.urlunsplit([scheme, authority, path, query, fragment])

    def set_proxy(self, host, t):
...
class OpenerDirector:
    ...
    def error(self, proto, *args):
        ...
        if proto in ('http', 'https'):
            ...
            _dict = self.handle_error['http']  # https is not different than http
            proto = args[2]  # YUCK!
            meth_name = 'http_error_%s' % proto
            http_err = 1
            orig_args = args
        else:
            _dict = self.handle_error
        ...
        if http_err:
            args = (_dict, 'default', 'http_error_default') + orig_args
...
class HTTPRedirectHandler(BaseHandler):
    ...
    def http_error_302(self, req, fp, code, msg, headers):
        ...
        if 'location' in headers:
            ...
            return
        newurl = rfc3986.clean_url(newurl, "latin-1")
        newurl = rfc3986.urljoin(req.get_full_url(), newurl)
...
class ProxyHandler(BaseHandler):
    ...
    def __init__(self, proxies=None, proxy_bypass=None):
        ...
        for t, url in proxies.items():
            setattr(self, '%s_open' % t,
                    lambda r, proxy=url, t=type, meth=self.proxy_open: \
                    meth(r, proxy, type))
        if proxy_bypass is None:
            proxy_bypass = urllib.proxy_bypass
        self._proxy_bypass = proxy_bypass

    def proxy_open(self, req, proxy, t):
...
class AbstractDigestAuthHandler:
    ...
    def get_authorization(self, req, chal):
        ...
        if qop == 'auth':
            ...
            # XXX handle auth-int.
            logger = logging.getLogger("pymechanize.auth")
...
class AbstractHTTPHandler(BaseHandler):
    ...
    def do_open(self, http_class, req):
        ...
        has methods and attributes including:
            - info(): return a Message object for the headers
...
class FTPHandler(BaseHandler):
    def ftp_open(self, req):
        ...
        try:
            ...
            msg = "ftp error: {0} {1}".format(e, sys.exc_info()[2])
            raise URLError(msg)
