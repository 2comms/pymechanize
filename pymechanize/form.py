...
# TODO:
# Clean up post the merge into pymechanize
#  * Remove code that was duplicated in ClientForm and pymechanize
...
_logger = logging.getLogger("pymechanize.forms")
...
def choose_boundary():
    ...
    nonce = "".join([str(random.randint(0, sys.maxsize - 1)) for i in range(3)])
    return "-"*27 + nonce
...
# TODO: move to _html
class ParseError(HTMLParseError):
...
class _AbstractFormParser:
    ...
    def handle_data(self, data):
        ...
        # not if within option or textarea
            ...
            _map = self._current_label
            key = "__text"
        else:
            return

        if data and not _map.has_key(key):
            # according to
...
class FormParser(_AbstractSgmllibParser, sgmllib_copy.SGMLParser):
    """Good for tolerance of incorrect HTML, bad for XHTML."""
    def __init__(self, entitydefs=None, encoding=DEFAULT_ENCODING):
        sgmllib_copy.SGMLParser.__init__(self)
        _AbstractFormParser.__init__(self, entitydefs, encoding)

    def feed(self, data):
        try:
            sgmllib_copy.SGMLParser.feed(self, data)
        except sgmllib_copy.SGMLParseError as e:
            raise ParseError(e)

    def close(self):
        sgmllib_copy.SGMLParser.close(self)
        self.end_body()
...
class NestingRobustFormParser(_AbstractBSFormParser, BeautifulSoup):
...
def ParseResponse(response, *args, **kwds):
    ...
    The return value of pymechanize.urlopen can be conveniently passed to this
    function as the response parameter.

    pymechanize.ParseError is raised on parse errors.
    ...
    encoding: character encoding used for encoding numeric character references
     when matching link text.  pymechanize does not attempt to find the encoding
     in a META HTTP-EQUIV attribute in the document itself (pymechanize, for
     example, does do that and will pass the correct value to pymechanize using
     this parameter).
    ...
    There is a choice of parsers.  pymechanize.XHTMLCompatibleFormParser (uses
    HTMLParser) works best for XHTML, pymechanize.FormParser (uses
...
class IsindexControl(ScalarControl):
    ...
    pymechanize.urlopen(form.click(type="isindex"))
    ...
    result = pymechanize.urlopen(url)
    ...
    def _click(self, form, coord, return_type, request_class=request.Request):
        ...
        parts = rest + (urllibparse.quote_plus(self.value), None)
...
class SelectControl(ListControl):
    ...
    OPTION 'values', in HTML parlance, are Item 'names' in pymechanize parlance.
...
class HTMLForm:
    ...
    passing to pymechanize.urlopen (or the click_request_data or click_pairs
    methods for integration with third-party code).

    import pymechanize
    forms = pymechanize.ParseFile(html, base_uri)
    form = forms[0]

    form["query"] = "Python"
    form.find_control("nr_results").get("lots").selected = True

    response = pymechanize.urlopen(form.click())
...
# Form submission methods, applying only to clickable controls.
    ...
    def click(self, name=None, type=None, id=None, nr=0, coord=(1, 1),
        ...
        The request object is a pymechanize.Request instance, which you can pass
        to pymechanize.urlopen.
    ...
    def click_request_data(self,
        ...
        if you're using httplib or urllib rather than pymechanize.  Otherwise,
        ...
        # just as much control with pymechanize.
    ...
    def click_pairs(self, name=None, type=None, id=None,
        ...
        pymechanize.  It may also be useful if you want to manually tweak the
        ...
        not using pymechanize, use click_request_data.
        """
...
# Private methods.
    ...
    def _request_data(self):
        ...
        if method == "GET":
            ...
            parts = rest + (urllibparse.urlencode(self._pairs()), None)
            ...
            uri = self._urlunparse(parts)
            if self.enctype == "application/x-www-form-urlencoded":
                return (uri, urllibparse.urlencode(self._pairs()),
    ...
    def _switch_click(self, return_type, request_class=request.Request):
