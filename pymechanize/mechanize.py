...
import copy, re, os, urllib

from .html import DefaultFactory
from . import response
from . import request
from . import rfc3986
from . import sockettimeout
from . import urllib_fork
from .useragent import UserAgentBase
...
class HTTPRefererProcessor(urllib_fork.BaseHandler):
...
class Browser(UserAgentBase):
    ...
    request: current request (pymechanize.Request)
    ...
    def __init__(self,
        ...
        factory: object implementing the pymechanize.Factory interface.
        history: object implementing the pymechanize.History interface.  Note
         this interface is still experimental and may change in future.
        request_class: Request class to use.  Defaults to pymechanize.Request
    ...
    def _add_referer_header(self, request, origin_request=True):
        ...
        if (self._handle_referer and
            ...
            parts = rfc3986.urlsplit(self.request.get_full_url())
            parts = parts[:-1] + (None,)
            referer = rfc3986.urlunsplit(parts)
    ...
    def _mech_open(self, url, data=None, update_history=True, visit=None,
        ...
        try:
            ...
            scheme, authority = rfc3986.urlsplit(url)[:2]
            if scheme is None:
                # relative URL
                ...
                url = rfc3986.urljoin(self._response.geturl(), url)
...
# #             raise
        ...
        if visit:
            ...
            response = response.upgrade_response(response)
    ...
    def visit_response(self, response, request=None):
        ...
        """
        if request is None:
            request = request.Request(response.geturl())
    ...
    def set_cookie(self, cookie_string):
        ...
        this method is necessary because pymechanize currently does not support
    ...
    def links(self, **kwds):
        """Return iterable over links (pymechanize.Link objects)."""
    ...
    def forms(self):
        ...
        The returned form objects implement the pymechanize.HTMLForm interface.
    ...
    def global_form(self):
        ...
        The returned form object implements the pymechanize.HTMLForm interface.
    ...
    def select_form(self, name=None, predicate=None, nr=None):
        ...
        If no matching form is found, pymechanize.FormNotFoundError is raised.
    ...
    def click(self, *args, **kwds):
        """See pymechanize.HTMLForm.click for documentation."""
        if not self.viewing_html():
            raise BrowserStateError("not viewing HTML")
    ...
    def submit(self, *args, **kwds):
        ...
        Arguments are as for pymechanize.HTMLForm.click().
    ...
    def find_link(self, **kwds):
        ...
        Links are returned as pymechanize.Link objects.
        ...
        If a matching link is not found, pymechanize.LinkNotFoundError is raised.
