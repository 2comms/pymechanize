...
from . import auth, gzip
from . import opener, response
from .sockettimeout import _GLOBAL_DEFAULT_TIMEOUT
from . import urllib


class UserAgentBase(opener.OpenerDirector):
    ...
    (e.g. pymechanize.Browser) can inherit from UserAgentBase.  The subclass
    ...
    handler_classes = {
        # scheme handlers
        "http": urllib.HTTPHandler,
        # CacheFTPHandler is buggy, at least in 2.3, so we don't use it
        "ftp": urllib.FTPHandler,
        "file": urllib.FileHandler,

        # other handlers
        "_unknown": urllib.UnknownHandler,
        # HTTP{S,}Handler depend on HTTPErrorProcessor too
        "_http_error": urllib.HTTPErrorProcessor,
        "_http_default_error": urllib.HTTPDefaultErrorHandler,

        # feature handlers
        "_basicauth": urllib.HTTPBasicAuthHandler,
        "_digestauth": urllib.HTTPDigestAuthHandler,
        "_redirect": urllib.HTTPRedirectHandler,
        "_cookies": urllib.HTTPCookieProcessor,
        "_refresh": urllib.HTTPRefreshProcessor,
        "_equiv": urllib.HTTPEquivProcessor,
        "_proxy": urllib.ProxyHandler,
        "_proxy_basicauth": urllib.ProxyBasicAuthHandler,
        "_proxy_digestauth": urllib.ProxyDigestAuthHandler,
        "_robots": urllib.HTTPRobotRulesProcessor,
        "_gzip": gzip.HTTPGzipProcessor,  # experimental!

        # debug handlers
        "_debug_redirect": urllib.HTTPRedirectDebugProcessor,
        "_debug_response_body": urllib.HTTPResponseDebugProcessor,
        }
    ...
    if hasattr(urllib, 'HTTPSHandler'):
        handler_classes["https"] = urllib.HTTPSHandler
        default_schemes.append("https")

    def __init__(self):
        opener.OpenerDirector.__init__(self)
...
# #         self._ftp_conn_cache = conn_cache
    ...
    def set_cookiejar(self, cookiejar):
        """Set a pymechanize.CookieJar, or None."""
    ...
    def add_client_certificate(self, url, key_file, cert_file):
        ...
        default behaviour of libopenssl.  In future pymechanize may support
    ...
    def set_password_manager(self, password_manager):
        """Set a pymechanize.HTTPPasswordMgrWithDefaultRealm, or None."""
    ...
    def set_proxy_password_manager(self, password_manager):
        """Set a pymechanize.HTTPProxyPasswordMgr, or None."""
    ...
    def set_client_cert_manager(self, cert_manager):
        """Set a pymechanize.HTTPClientCertMgr, or None."""
    ...
    def set_debug_redirects(self, handle):
        ...
        "pymechanize.http_redirects".  To actually print some debug output,
        eg:

        import sys, logging
        logger = logging.getLogger("pymechanize.http_redirects")
        logger.addHandler(logging.StreamHandler(sys.stdout))
        logger.setLevel(logging.INFO)

        Other logger names relevant to this module:

        "pymechanize.http_responses"
        "pymechanize.cookies"
...
class UserAgent(UserAgentBase):
    ...
    def open(self, fullurl, data=None,
        ...
        if self._seekable:
            ...
            response = opener.wrapped_open(
                bound_open, response.seek_wrapped_response, fullurl, data,
                timeout)
        else:
