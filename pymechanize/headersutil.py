...
import os, re
from .util import http2time
from .import rfc3986

def is_html_file_extension(url, allow_xhtml):
    ext = os.path.splitext(rfc3986.urlsplit(url)[2])[1]
    html_exts = [".htm", ".html"]
    if allow_xhtml:
        html_exts += [".xhtml"]
...
def split_header_words(header_values):
    ...
    """
    assert type(header_values) not in [bytes, str]
    result = []
...
def _test():
    pass
#     import doctest
#     from . import headersutil
#     return doctest.testmod(headersutil)
