...
import sys, re, copy, time, logging
...
from pymechanize.headersutil import split_header_words, parse_ns_headers
from pymechanize.util import isstringlike
from . import USE_BARE_EXCEPT, rfc3986
from .util import cmp

debug = logging.getLogger("pymechanize.cookies").debug
...
def request_path(request):
    ...
    url = request.get_full_url()
    path = escape_path(rfc3986.urlsplit(url)[2])
...
class CookiePolicy:
    ...
    def set_ok(self, cookie, request):
        ...
        cookie: pymechanize.Cookie object
...
class DefaultCookiePolicy(CookiePolicy):
    ...
    import pymechanize
    class MyCookiePolicy(pymechanize.DefaultCookiePolicy):
        def set_ok(self, cookie, request):
            if not pymechanize.DefaultCookiePolicy.set_ok(
                self, cookie, request):
                return False
...
class CookieJar:
    ...
    You may not need to know about this class: try pymechanize.urlopen().
    ...
    def add_cookie_header(self, request):
        """Add correct Cookie: header to request (pymechanize.Request object).

        The Cookie2 header is also added unless policy.hide_cookie2 is true.

        The request object (usually a pymechanize.Request instance) must support
    ...
    def set_cookie_if_ok(self, cookie, request):
        ...
        cookie: pymechanize.Cookie instance
    ...
    def extract_cookies(self, response, request):
        ...
        pymechanize.urlopen, or similar) should support an info method, which
        ...
        documented by pymechanize, and the port attribute (the port number).  The
